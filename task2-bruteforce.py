...
if __name__ == "__main__":
    ...
    for alpha in alphas:
        ...
        '''Mallory Attack Below: Brute Force'''
        # the values we know: public keys: yA, yB, alpha, q, q-1
        
        # attempt to find private key for A
        exA = 1
        sA = pow(yB, exA, int(q.replace(" ", ""), 16))
        while sA != yA:
            exA += 1
            sA = pow(yB, exA, int(q.replace(" ", ""), 16)) 
            
        # attempt to find private key for B
        exB = 1
        sB = pow(yA, exB, int(q.replace(" ", ""), 16))
        while sB != yB:
            exB += 1
            sB = pow(yA, exB, int(q.replace(" ", ""), 16)) 
